 ...
 * owner membership): the tenant owner-set lock (0013) is taken first, then all
 ...
 * first revocation (the 0012/0013 memberships trigger enforces it in PostgreSQL). The last active owner
...
export function createMembershipRevocationHandler(options: MembershipRevocationOptions = {}): OutboxHandler {
  return async (event, tx) => {
    ...
    // S1-05 audit fix: take the tenant owner-set lock (0013: owner gate +
    // this job's tenant workshop row, derived from the bound TenantContext)
    // BEFORE any row lock, in the same order as the role commands (no
    // lock-order deadlock). The owner list below is then read on a snapshot
    // taken after the wait, so a concurrent owner-role removal that committed
    // meanwhile is seen. The memberships trigger re-checks it in PostgreSQL.
