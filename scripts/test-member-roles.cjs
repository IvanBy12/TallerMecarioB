...
/**
 * S1-05 member role management suites against a REAL, ephemeral, local-only
 ...
 *   - compiles src/ to a temp dir and runs tests/member-roles/*.test.cjs
...
const { applyMutation } = require('./member-roles-mutations.cjs');
...
async function main() {
  ...
  const databaseName = `tallermecario_member_roles_${suffix}`;
  const apiLogin = `tm_test_mrapi_${suffix}`;
  const workerLogin = `tm_test_mrwrk_${suffix}`;
  ...
  try {
    ...
    compiledRoot = mkdtempSync(join(tmpdir(), 'tallermecario-member-roles-test-'));
    runChild([
    ...
    const files = readdirSync('tests/member-roles')
      .filter((name) => name.endsWith('.test.cjs'))
      .sort()
      .map((name) => `tests/member-roles/${name}`);
    ...
    if (compiledRoot && compiledRoot.startsWith(join(tmpdir(), 'tallermecario-member-roles-test-'))) {
  ...
  if (!testPassed) throw new Error('MEMBER_ROLE_TESTS_FAILED');
  if (!cleanupPassed) throw new Error('TEST_DATABASE_CLEANUP_FAILED');
  process.stdout.write('MEMBER_ROLE_FIXTURE_CLEANUP_PASS\n');
}
