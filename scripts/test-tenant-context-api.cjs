...
/**
 * S1-02 TenantContext + RBAC Fastify integration against a REAL, ephemeral,
 * local-only PostgreSQL database: creates a throwaway database, runs the full
 * migration chain from empty (drizzle/*.sql via scripts/migrate.cjs — never
 * drizzle-kit push), creates a throwaway NOBYPASSRLS login that is a NOINHERIT
 * member of tallermecario_api, compiles src/ to a temp dir, runs
 * tests/api/tenant-context-integration.test.cjs, then tears everything down
 * and fails if cleanup did not complete. Mirrors scripts/test-api-multitenant.cjs.
 *
...
function runChild(args, env, timeout = 180_000) {
...
async function main() {
  ...
  const databaseName = `tallermecario_tenant_api_${suffix}`;
  const loginRole = `tm_test_tenapi_${suffix}`;
  ...
  try {
    ...
    compiledRoot = mkdtempSync(join(tmpdir(), 'tallermecario-tenant-api-test-'));
    runChild([
    ...
    if (compiledRoot && compiledRoot.startsWith(join(tmpdir(), 'tallermecario-tenant-api-test-'))) {
  ...
  if (!testPassed) throw new Error('TENANT_CONTEXT_API_TESTS_FAILED');
  if (!cleanupPassed) throw new Error('TEST_DATABASE_CLEANUP_FAILED');
  process.stdout.write('TENANT_CONTEXT_API_FIXTURE_CLEANUP_PASS\n');
}
...
main().catch((error) => {
  const safeMessages = new Set([
    ...
    'TENANT_CONTEXT_API_TESTS_FAILED',
    'TEST_DATABASE_CLEANUP_FAILED',
  ]);
  process.stderr.write(`${safeMessages.has(error.message) ? error.message : 'TENANT_CONTEXT_API_TEST_RUN_FAILED'}\n`);
  process.exitCode = 1;
});
