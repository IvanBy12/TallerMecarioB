...
/**
 * S1-06 membership lifecycle suites against a REAL, ephemeral, local-only
 ...
 *   - compiles src/ to a temp dir and runs tests/member-lifecycle/*.test.cjs
 *     sequentially (JWTs signed with per-run RSA keys, fake Backend API);
 *   - optional S106_MUTATION (scripts/member-lifecycle-mutations.cjs) breaks
 *     the compiled JS / test DB on purpose; the mutation runner expects a FAIL;
...
const { applyMutation } = require('./member-lifecycle-mutations.cjs');
...
const COMPILE_PREFIX = 'tallermecario-member-lifecycle-test-';
...
async function main() {
  ...
  const databaseName = `tallermecario_member_lifecycle_${suffix}`;
  const apiLogin = `tm_test_mlapi_${suffix}`;
  const workerLogin = `tm_test_mlwrk_${suffix}`;
  ...
  try {
    ...
    const files = readdirSync('tests/member-lifecycle')
      .filter((name) => name.endsWith('.test.cjs'))
      .sort()
      .map((name) => `tests/member-lifecycle/${name}`);
  ...
  if (!testPassed) throw new Error('MEMBER_LIFECYCLE_TESTS_FAILED');
  if (!cleanupPassed) throw new Error('TEST_DATABASE_CLEANUP_FAILED');
  process.stdout.write('MEMBER_LIFECYCLE_FIXTURE_CLEANUP_PASS\n');
}
...
main().catch((error) => {
  ...
  process.stderr.write(`${safeMessages.has(error.message) || mutationError ? error.message : 'MEMBER_LIFECYCLE_TEST_RUN_FAILED'}\n`);
  process.exitCode = 1;
});
