...
/**
 * S2-04 Customers API suites against a REAL, ephemeral, local-only
 ...
 *   - compiles src/ to a temp dir and runs tests/crm-api/*.test.cjs
 *     sequentially (JWTs signed with per-run RSA keys, fake Backend API);
 *   - optional S204_MUTATION (scripts/crm-api-mutations.cjs) breaks
...
async function main() {
  ...
  const databaseName = `tallermecario_crm_api_${suffix}`;
  const apiLogin = `tm_test_crmapi_${suffix}`;
  const workerLogin = `tm_test_crmwrk_${suffix}`;
...
main().catch((error) => {
  ...
  process.stderr.write(`${safeMessages.has(error.message) || mutationError ? error.message : 'CRM_API_TEST_RUN_FAILED'}\n`);
  process.exitCode = 1;
});
