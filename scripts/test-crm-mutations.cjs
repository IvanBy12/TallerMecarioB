...
for (const [name,[anchor,replacement]] of selected) {
  ...
  try {
    ...
    const script = name==='M8' ? 'scripts/test-crm-migration-upgrade.cjs' : 'scripts/test-crm-db.cjs';
    ...
    const killed = r.status !== 0 && (migrationRejected || testRejected || preflightRejected);
    process.stdout.write(`${name} ${killed?'KILLED':'SURVIVED'}${output.includes('CRM_TEARDOWN dbs=0 logins=0') || output.includes('CRM_UPGRADE_TEARDOWN dbs=0') ? ' teardown=clean':''}\n`);
...
else process.stdout.write(`CRM_MUTATIONS_PASS ${selected.length}/${selected.length} killed\n`);
