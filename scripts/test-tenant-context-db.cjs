...
/**
 * S1-02 TenantContext DB foundation against a REAL, ephemeral, local-only
 * PostgreSQL database: creates a throwaway database, runs the full migration
 * chain from empty (drizzle/*.sql via scripts/migrate.cjs — never
 ...
 * tests/tenancy/tenant-context-db.test.cjs, then tears everything down and
 * fails if cleanup did not complete. Mirrors scripts/test-authz-db.cjs.
 *
...
function runChild(args, env, timeout = 120_000) {
...
async function main() {
  ...
  const databaseName = `tallermecario_tenant_ctx_${suffix}`;
  const loginRole = `tm_test_tenctx_${suffix}`;
  ...
  try {
    ...
    compiledRoot = mkdtempSync(join(tmpdir(), 'tallermecario-tenant-ctx-db-test-'));
    runChild([
    ...
    testArgs.push('tests/tenancy/tenant-context-db.test.cjs');
    runChild(testArgs, {
      ...process.env,
      TEST_DATABASE_URL_ADMIN: testUrl.toString(),
      TEST_RUNTIME_LOGIN: loginRole,
      TEST_RUNTIME_PASSWORD: loginPassword,
      TEST_TENANCY_MODULE_ROOT: compiledRoot,
    });
    testPassed = true;
  } finally {
    if (testAdmin) await testAdmin.end({ timeout: 5 }).catch(() => undefined);

    if (compiledRoot && compiledRoot.startsWith(join(tmpdir(), 'tallermecario-tenant-ctx-db-test-'))) {
  ...
  if (!testPassed) throw new Error('TENANT_CONTEXT_DB_TESTS_FAILED');
  if (!cleanupPassed) throw new Error('TEST_DATABASE_CLEANUP_FAILED');
  process.stdout.write('TENANT_CONTEXT_DB_FIXTURE_CLEANUP_PASS\n');
}
...
main().catch((error) => {
  const safeMessages = new Set([
    ...
    'TENANT_CONTEXT_DB_TESTS_FAILED',
    'TEST_DATABASE_CLEANUP_FAILED',
  ]);
  process.stderr.write(`${safeMessages.has(error.message) ? error.message : 'TENANT_CONTEXT_DB_TEST_RUN_FAILED'}\n`);
  process.exitCode = 1;
});
