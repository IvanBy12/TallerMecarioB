...
/**
 * S2-04 mutation run: executes the customers API suite once per mutant
 * (scripts/crm-api-mutations.cjs) and requires every run to FAIL.
 * A surviving mutant fails this script. Local database only (same guards as
 * scripts/test-crm-api.cjs).
 */

const { spawnSync } = require('node:child_process');
const { MUTATIONS } = require('./crm-api-mutations.cjs');

const names = process.env.S204_MUTATIONS ? process.env.S204_MUTATIONS.split(',') : Object.keys(MUTATIONS);
const results = [];

for (const name of names) {
  const run = spawnSync(process.execPath, ['scripts/test-crm-api.cjs'], {
    cwd: process.cwd(),
    env: { ...process.env, S204_MUTATION: name, NO_COLOR: '1', FORCE_COLOR: '0' },
    encoding: 'utf8',
    timeout: 600_000,
  });
  ...
  const applied = output.includes(`S204_MUTATION_APPLIED ${name}`);
