...
/**
 * S1-08 cross-tenant final suites against a REAL, ephemeral, local-only
 ...
 *   - compiles src/ to a temp dir and runs tests/cross-tenant/*.test.cjs
 *     sequentially (JWTs signed with per-run RSA keys, fake Backend API,
 *     recording email sender);
 *   - optional S108_MUTATION breaks compiled JS / test DB in a throwaway copy;
...
const { applyMutation } = require('./cross-tenant-mutations.cjs');
...
const COMPILE_PREFIX = 'tallermecario-s108-test-';
...
async function main() {
  ...
  const databaseName = `tallermecario_s108_${suffix}`;
  const apiLogin = `tm_test_xapi_${suffix}`;
  const workerLogin = `tm_test_xwrk_${suffix}`;
  ...
  try {
    ...
    const files = readdirSync('tests/cross-tenant')
      .filter((name) => name.endsWith('.test.cjs'))
      .sort()
      .map((name) => `tests/cross-tenant/${name}`);
  ...
  if (!testPassed) throw new Error('CROSS_TENANT_TESTS_FAILED');
  if (!cleanupPassed) throw new Error('TEST_DATABASE_CLEANUP_FAILED');
  process.stdout.write('CROSS_TENANT_FIXTURE_CLEANUP_PASS\n');
}
...
main().catch((error) => {
  ...
  process.stderr.write(`${safeMessages.has(error.message) || mutationError ? error.message : 'CROSS_TENANT_TEST_RUN_FAILED'}\n`);
  process.exitCode = 1;
});
