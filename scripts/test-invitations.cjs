...
/**
 * S1-04 membership invitation suites against a REAL, ephemeral, local-only
 ...
 *   - compiles src/ to a temp dir and runs tests/invitations/*.test.cjs
...
async function main() {
  ...
  const databaseName = `tallermecario_invitations_${suffix}`;
  const apiLogin = `tm_test_invapi_${suffix}`;
  const workerLogin = `tm_test_invwrk_${suffix}`;
  ...
  try {
    ...
    compiledRoot = mkdtempSync(join(tmpdir(), 'tallermecario-invitations-test-'));
    runChild([
      resolve('node_modules/typescript/bin/tsc'), '-p', 'tsconfig.json', '--noEmit', 'false',
      '--rootDir', 'src', '--outDir', compiledRoot,
    ], process.env);

    const files = readdirSync('tests/invitations')
      .filter((name) => name.endsWith('.test.cjs'))
      .sort()
      .map((name) => `tests/invitations/${name}`);
    ...
    if (compiledRoot && compiledRoot.startsWith(join(tmpdir(), 'tallermecario-invitations-test-'))) {
  ...
  if (!testPassed) throw new Error('INVITATION_TESTS_FAILED');
  if (!cleanupPassed) throw new Error('TEST_DATABASE_CLEANUP_FAILED');
  process.stdout.write('INVITATION_FIXTURE_CLEANUP_PASS\n');
}
...
main().catch((error) => {
  const safeMessages = new Set([
    ...
    'INVITATION_TESTS_FAILED',
    'TEST_DATABASE_CLEANUP_FAILED',
  ]);
  process.stderr.write(`${safeMessages.has(error.message) ? error.message : 'INVITATION_TEST_RUN_FAILED'}\n`);
  process.exitCode = 1;
});
