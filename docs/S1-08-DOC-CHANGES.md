...
Preparado el 2026-09-25 en `task/s1-08-cross-tenant-final`, base `task/s1-07-audit` (`d873463`). **No fusionado con los documentos canónicos.** Aplicar por secciones después del audit de arquitectura, preservando S1-03…S1-07.
...
| Operación v1 | §5.1–5.2 y worker/outbox | Añadir la verificación de coherencia entre claim y fila durable antes del handler o de marcar `processed`: deben coincidir `outboxEventId` y `tenantId`. Un mismatch falla sin efecto sobre el recurso ni avance del job ajeno. Conservar la atomicidad business + success audit y el catálogo S1-07. |
...
- Las denegaciones de selección cross-tenant no crean auditoría en B. Los denegados de dominio conservan la política S1-07. Un ID B invisible no debe aparecer como `entity_id` ni en metadata de una operación de A.
...
- Suite dedicada: `test:cross-tenant:final`, **24/24 PASS**, PostgreSQL real descartable y limpieza de DB/logins.
- Mutaciones: `test:cross-tenant:mutations`, **8/8 KILLED**: desactivar RLS; quitar FORCE; omitir tenant GUC; confiar en tenant del header; quitar FK compuesta `assigned_by`; GUC de sesión tras error; helper de lock con tenant UUID arbitrario; omitir comparación claim/tenant del worker. Las mutaciones solo afectan DB descartable o JS compilado temporal, no el source.
...
- **DOC_CONFLICT: ninguno nuevo.** La corrección Security Baseline §19 ↔ Operación §5.1 sobre JIT/IP/User-Agent consta en S1-07; esta propuesta la preserva. La fusión canónica de S1-07 sigue siendo un paso documental separado.
...
Esta evidencia prepara el audit de arquitectura S1-08; no declara `PASSED` el Quality Gate final de Sprint 1.
