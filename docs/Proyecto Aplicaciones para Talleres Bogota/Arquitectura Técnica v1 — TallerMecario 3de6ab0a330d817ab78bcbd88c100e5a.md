...
- **Auditoría** (Operación §5.2): `customer.created`, `customer.updated`, `vehicle.created`, `vehicle.updated` y `vehicle.owner_changed`, en la misma transacción y con actor del TenantContext. No se auditan lecturas, 404, 409, `PERMISSION_DENIED`, el PATCH no-op de customer ni el no-op de propietario. `fields`/`changed_fields` de customer usan nombres de columna snake_case (Operación §5.2).
